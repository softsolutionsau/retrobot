...
function notInRetro(message) {
  const text = 'The retrospective isn\'t runnning at the moment. Here, have ' +
    'some wine whilst you wait for it to start ðŸ·';
...
function command(message) {
  ...
  if (aliases[cmd] === start) {
    ...
    let msg = 'ðŸ”” Retrospective is now recording';
        ...
        // slack.dnd.info({ token, user }, (error, dnd) => {
          ...
          // slack.users.getPresence({ token, user }, (error, presence) => {
            ...
            // slack.im.open({ token, user }, (error, data) => {
              ...
              // reply({ channel: dms[user] }, 'ðŸ‘‚ Tell me about your retrospective' +
...
function stopRetro(message) {
  if (inRetro === false) {
    return reply(message, 'The retrospective finished already. I don\'t have a historical record of the retro to print here either. Sorry. Have some tea ðŸµ');
  }
...
function flushRetrospective(retrospective) {
  ...
  messages.push(`*Below is the "worked well" list, good work team ðŸ‘*`);
  plus.map((res, i) => {
    messages.push(`${i + 1}. ${res.line}`);
  });

  messages.push(`*Below is the "needs work" list, ðŸŒ±*`);
  minus.map((res, i) => {
    messages.push(`${i + 1}. ${res.line}`);
  });

  messages.push(`\n*Please go ahead and use ðŸ‘ to indicate that you think a particular item should become an action item for the next sprint.*\n\nSee you after the next sprint ðŸ‘‹ðŸ”Œ`);
